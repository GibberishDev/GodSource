...
## Color of outlines
@export var outline_color : Color = Color.GOLD
...
@export var stencil_value : int = 1
...
var jf_shader: RID
var jf_pipeline: RID
var jf_uniform_sets : Array = [RID(), RID()]
...
func _init() -> void:
	...
	var vertex_attr : RDVertexAttribute = RDVertexAttribute.new()
...
func _load_glsl_from_file(path: String) -> Variant:
...
func _build_sc_pipeline() -> void:
	...
	# load the shader
	var shader_spirv : RDShaderSPIRV = _load_glsl_from_file(sc_shader_file)
	if not shader_spirv:
	...
	# fragment
	var stencil_state : RDPipelineDepthStencilState = RDPipelineDepthStencilState.new()
...
func _build_do_pipeline() -> void:
	...
	# load the shader
	var shader_spirv : RDShaderSPIRV = _load_glsl_from_file(do_shader_file)
	if not shader_spirv:
		push_error("failed to load stencil copy shader")
		return
...
func _build_jf_pipeline() -> void:
	...
	var shader_spirv : RDShaderSPIRV = _load_glsl_from_file(jf_shader_file)
	if not shader_spirv:
	...
	for group : Array in [[0, _textures[0], _textures[1]],
...
## pipelines
func _update_common_buffers() -> void:
