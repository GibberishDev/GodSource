...
func start_server(port: int, slots: int) -> void:
	...
	if port >= 1024 and port <= 65535:
		...
		GSConsole.send_output_message("[color=lime]Created server at port " + str(port) + "[/color]")
		...
		GSConsole.send_output_message("[color=red]Failed to create server. ERROR: Invalid port number [b]" + str(port) + ". Expected a number between 1024 and 65535[/b][/color]")
		return
...
func _peer_connected(id: int) -> void:
	GSConsole.send_output_message("[color=green]+++[/color] Peer connected.	Peer id: " + str(id))

func _peer_disconnected(id: int) -> void:
	GSConsole.send_output_message("[color=red]---[/color] Peer disconnected.	Peer id: " + str(id))

func _peer_connected_to_server() -> void:
	GSConsole.send_output_message("[color=green]Connected to server[/color]")

func _peer_disconnected_from_server() -> void:
	GSConsole.send_output_message("[color=red]Disconnected from server[/color]")

func _client_failed_to_connect() -> void:
	GSConsole.send_output_message("[color=red]Failed to connect to server[/color]")
